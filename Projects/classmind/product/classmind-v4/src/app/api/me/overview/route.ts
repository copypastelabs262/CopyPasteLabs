...
export async function GET() {
  try {
    ...
    const recentConversations = recentConvos.conversations
      // Global threads (courseId null) always belong here -- they ARE this
      // page's own scope. Course/lecture threads only while the student can
      // still open their course.
      ...
      .map((c) => {
        const entry = c.courseId ? courseById.get(c.courseId) : undefined;
        return {
          id: c.id,
          title: c.title,
          scope: c.scope,
          courseId: c.courseId,
          courseCode: entry ? entry.course.code : "All subjects",
