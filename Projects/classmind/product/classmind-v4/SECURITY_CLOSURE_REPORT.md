...
**Date:** 2026-09-07 · **Scope as written:** the disposable audit copy `classmind-v4 - sECURE aUDIT`
**Ported into this tree — the tracked, deployable `classmind-v4` — on 2026-09-09.** Nothing pushed.
Nothing deployed. Zero paid provider calls, in either pass.
...
2. ~~**Port the credential fix to the real tree.**~~ **DONE 2026-09-07** (commit `c09ed7c`):
   ...
   `Projects/classmind/product/classmind-v4/`, with `CLASSMIND_TEST_PASSWORD` in its `.env.local`
   and as an empty key in `.env.example`. The rest of the security pass followed on **2026-09-09**.
...
migrations are unapplied**, so four schema-level controls are intent rather than state; and the
ASR duration ceiling and the pre-staging bypass are unfixed by choice.

**This is a credible security baseline** — meaning: a documented threat model, 911 passing
