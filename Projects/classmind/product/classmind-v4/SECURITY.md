...
Written 2026-09-07 during an autonomous security audit of a disposable copy of this
tree, and **ported into this tree on 2026-09-09** — the tracked, deployable one. It
describes where the boundaries actually are, not where they ought to be, and it names
what is still open.
...
## READ THIS FIRST — the two things code cannot fix

**1. The published test password must never be reused.** `faculty.test@classmind.local`
and `student.test@classmind.local` once shared a password that was a literal in ~50
files tracked at HEAD on `github.com/copypastelabs262/CopyPasteLabs`, which is public.
The accounts were **deleted and verified absent from the live project on 2026-09-07**
(`DEPLOY.md` §6), and every script now reads the credential from
`CLASSMIND_TEST_PASSWORD` through `scripts/_test-credentials.mts`. What cannot be
undone is the history: the string stands in 21 commits across four project versions,
and only a history rewrite removes it. Treat it as burned. Note also that the
verification suites **re-create** those two accounts on their next run, so set a fresh
`CLASSMIND_TEST_PASSWORD` before running any of them.

**2. Two migrations are written and unapplied.**
`20260907120000_security_hardening.sql` and `20260907130000_ledger_durability.sql` sit
in `supabase/migrations/`, and no documented deploy step applies migrations at all.
Until they run, four schema-level controls described below are intent rather than
state — including dropping the `profiles.role` default of `'faculty'`. Both migrations
`raise exception` rather than apply partially, so a silent half-application is not
possible. Procedure and pre-flight queries: `SECURITY_CLOSURE_REPORT.md` §17.
