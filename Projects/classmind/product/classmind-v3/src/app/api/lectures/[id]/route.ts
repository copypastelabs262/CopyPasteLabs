...
import { fetchLectureGateRows, lectureVisibleToStudents, lectureWithheldReason } from "@/lib/knowledge/read";

// One lecture, role-aware.
//
// Owner: transcript plus every candidate and its review history.
// Enrolled student: transcript only, once the lecture is `ready` AND its
// transcript can be shown to have come from this recording -- candidates never
// appear in this payload for a non-owner, which is why an unconfirmed item
// cannot reach a student even by guessing a URL.
//
// This is the ONLY route that serves a transcript, the raw provider response
// and a signed audio URL, so it is the route where a replayed lecture leaks the
// most: the whole of somebody else's lecture, verbatim, with playable audio.
// The check is the same one the knowledge funnel uses, imported rather than
// restated.
export async function GET(_r: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Withheld BEFORE anything is read, signed or normalized. Refusing early is
    // what makes the guarantee checkable: there is no branch below this line on
    // which a non-owner holds a transcript, a raw response or an audio URL for
    // a lecture that failed the gate. A partial payload -- metadata now,
    // transcript nulled out later -- would depend on every future field being
    // remembered, and one of them eventually would not be.
    if (!isOwner) {
      const [gate] = await fetchLectureGateRows({ ids: [id] });
      if (!gate || !lectureVisibleToStudents(gate)) {
        return NextResponse.json(
          {
            error:
              "This lecture is being verified and is not available yet. " +
              "Its transcript has not been shown to come from this recording.",
            withheld: true,
            // Safe to state: it describes the RECORD, not its contents. A
            // student who knows why can ask their lecturer, which is the
            // outcome we want; silence would just look like a bug.
            reason: gate ? lectureWithheldReason(gate) : "This lecture's provenance could not be read.",
          },
          { status: 403 },
        );
      }
