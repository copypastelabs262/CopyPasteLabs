...
async function submit(
  ...
  // The name Sarvam stores this input under is OUR lecture id, not the name
  // the uploader typed. Three consequences, one change: no user-controlled
  // string reaches the provider request; the output we download back is
  // provably the output of the file we named; and there is no longer a user
  // filename in the system for anything to key a transcript choice off.
  const name = providerFilename(audio);
  ...
  const uploads = (await call("/upload-files", {
    method: "POST",
    body: JSON.stringify({ job_id: created.job_id, files: [name] }),
  })) as { upload_urls?: Record<string, { file_url?: string }> };

  const target = uploads.upload_urls?.[name];
  if (!target?.file_url) {
    throw new Error("Sarvam returned no upload URL for " + name + ".");
  }
...
// Sarvam echoes an `audio_hash` on its batch responses -- present on all five
// genuine production jobs observed on 2026-08-30, absent on every fixture
// replay, seed and pending upload. Reliable enough for the live provider to
// depend on, optional enough that the type must stay nullable.
//
// This is the ONLY place the field name is known. The routes ask the adapter;
// they never reach into the response themselves. Read defensively because the
// artefact is authoritative and its shape is the provider's to change.
//
// Deliberately not overclaimed: this is a hash of Sarvam's DECODED audio, not
// of the bytes we uploaded. It cannot prove a transcript is ours. It can only
// tell us we have seen this decoded audio before.
function audioIdentity(raw: unknown): string | null {
