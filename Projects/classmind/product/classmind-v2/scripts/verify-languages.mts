...
//
// Requires the dev server on BASE. Each lecture NAMES the captured response it
// replays (`replayFixture: "<slug>"`), so no paid Sarvam call is made.
//
// It used to work by setting TRANSCRIPTION_PROVIDER=fixture and calling the
// uploaded file after the fixture slug -- selection by filename, which is the
// mechanism that served a thermodynamics transcript to a lecture called
// "Cloud computing.mp3" on 2026-08-22. Both are gone: the env var is read
// nowhere, and the filenames below are now deliberately NOT fixture slugs, so
// this suite proves the slug it names is what arrives.
//
...
// Deliberately NOT the slug. Nothing keys off it any more; the slug is passed
// explicitly at lecture creation and this filename proves it is not needed.
const HINDI_FILENAME = "Physics unit 1 recording.mp3";
...
const MISDETECTED_FILENAME = "DSP week 3 recording.mp3";
...
// The extract route runs a multi-call reasoning pass and, on a 34,000-character
// Hindi transcript, takes longer than 300 seconds -- which is undici's
// `headersTimeout`, a CLIENT-side limit with no public knob in Node and nothing
// to do with whether the server succeeded. Retrying does not help: every call
// redoes the reasoning. So this one long request goes over node:http, which
// imposes no such deadline. Built in; no dependency is added.
...
async function runLecture(opts: {
  ...
  audioPath: string;
  filename: string;
  // The captured response this lecture replays, named explicitly. There is no
  // other way to reach the fixture provider any more.
  ...
  const lec = await api(opts.token, "POST", `/api/courses/${courseId}/lectures`, {
    ...
    replayFixture: opts.replayFixture,
  });
  check("lecture row and signed upload URL", lec.status === 200 && !!lec.json?.signedUrl, lec.json);
  await assertReplayStuck(lec.json.lectureId as string, opts.replayFixture, lec.json);
  ...
  // expectReplay can only cause a refusal, never a replay: it makes a lost
  // replay request abort instead of falling through to a billable live call.
...
async function main() {
  ...
  try {
    hiExtract = await postLongRunning(faculty.token, `/api/lectures/${hi.lectureId}/extract`);
  } catch (err) {
  ...
  const mis = await runLecture({
    ...
    replayFixture: MISDETECTED_SLUG,
    lectureTitle: "8-point DIT FFT algorithm",
  });

  // UPDATED for the transcript quarantine guard added in migration
  // 20260830100000. This suite predates it and used to assert that the
  // romanized-Arabic fixture reached `transcribed` -- i.e. that a fluent
  // wrong-language transcript was accepted and published. That is precisely the
  // behaviour the guard exists to stop, so the assertion is not relaxed, it is
  // INVERTED: the lecture must now be QUARANTINED, and the raw artefact must
  // still be retained (checks 11 below prove that separately).
  check(
    "8. the wrong-language lecture is QUARANTINED, not published",
    mis.row.status === "quarantined",
    mis.row.status,
  );
  ...
  // UPDATED, and this one is the more important correction of the two.
  //
  // This used to require a "reported language X but configured Y" warning OR a
  // low-confidence warning. Both were shown to be worthless on the very run
  // this section tests: the engine reported en-IN, exactly what it was
  // configured with, so the mismatch check compares two fields that agree by
  // construction; and live batch responses carry no language_probability at
  // all, so the confidence check only ever fired on captured fixtures. Asserting
  // that a warning nobody can rely on is present would keep a dead guard alive.
  //
  // What must be present is the verdict from the check that DOES read the
  // transcript. Strictly stronger: it fires on this fixture, and it fires on
  // live responses where neither of the two above can.
  const languageVerdict = limitations.some((l) =>
    /not in English, Hindi or Hinglish|not a supported language|too short to confirm|script this product does not support/i.test(l),
  );
  check(
    "9. limitations carry the transcript guard's verdict, in words a reader can act on",
    languageVerdict,
    limitations,
  );
  console.log(
    `        9. transcript-guard verdict: ${languageVerdict ? "present" : "ABSENT"}; ` +
      `(legacy) mismatch warning: ${mismatchWarning ? "present" : "absent"}; ` +
