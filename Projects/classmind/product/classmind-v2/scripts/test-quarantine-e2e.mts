...
//
//   node --env-file=.env.local scripts/test-quarantine-e2e.mts
//
...
//
// The bad lecture NAMES the fixture it wants -- replayFixture:
// "fft-lecture-misdetected" -- so it replays the captured Sarvam response that
// returned fluent romanized ARABIC for an English DSP lecture while reporting
// language_code "en-IN". That is a real provider response, not a synthesised one.
//
// It used to name it by calling the UPLOADED FILE after the fixture slug, with
// TRANSCRIPTION_PROVIDER=fixture set in the environment. Both mechanisms are
// gone: a filename now selects nothing, and there is no env var. The uploads
// below are therefore given names that match no fixture at all, which makes
// this suite prove the new mechanism rather than merely survive it.
...
// Any real audio object will do: the replayed transcript is chosen by the slug
// this suite names, not by the bytes and no longer by the filename. The bytes
// still travel the real upload path so nothing about the flow is skipped.
...
async function uploadLecture(
  ...
  // The replay must be recorded against THIS lecture by name. If it were not,
  // the transcribe call below would make a live, billable Sarvam request and
  // every assertion after it would be measuring the wrong thing.
  await assertReplayStuck(lec.json.lectureId as string, replayFixture, lec.json, ` for "${title}"`);
...
async function runToTerminal(token: string, lectureId: string) {
  // expectReplay can only cause a refusal, never a replay: it asserts the
  // replay request is still known to the server, so losing it aborts rather
...
async function main() {
  ...
  console.log("transcription requested: explicit per-lecture replay (no env var, no filename)");
  console.log("        (what actually happened is printed per lecture below)");
  ...
  const badId = await uploadLecture(
    faculty.token, course.id, "Lecture 1 recording.mp3", "Lecture 1 - 8-point DIT FFT",
    "fft-lecture-misdetected",
  );
  ...
  const goodId = await uploadLecture(
    faculty.token, course.id, "Lecture 2 recording.mp3", "Lecture 2 - Cloud control layer",
    "cloud-computing-hinglish",
  );
  ...
  const goodExt = await postLongRunning(faculty.token, `/api/lectures/${goodId}/extract`);
  ...
  const { count: goodKnowledge } = await svc.from("knowledge_items")
    .select("id", { count: "exact", head: true }).eq("lecture_id", goodId);
  const published = goodAfter?.status === "ready";

  // THE INVARIANT, not the happy path.
  //
  // Publication depends on a model call, so asserting "the lecture reaches
  // ready" makes this suite flap whenever the provider has a bad minute -- a
  // test that fails for reasons unrelated to the code under test teaches people
  // to ignore it. What must ALWAYS hold is narrower and more important: a
  // lecture is never published empty. Publishing an empty lecture is confident
  // success over an empty result, which is the same failure shape as the Arabic
  // transcript.
  check(
    "a lecture is NEVER published with zero knowledge",
    !(published && (goodKnowledge ?? 0) === 0),
    { status: goodAfter?.status, knowledgeItems: goodKnowledge },
  );
  check(
    "and when it is NOT published, the reason is stated rather than left blank",
    published || typeof goodExt.json?.readiness?.reason === "string",
    goodExt.json?.readiness,
  );
  if (published) {
    console.log(`        published with ${goodKnowledge} knowledge items`);
  } else {
    console.log(`        NOT published -- ${goodExt.json?.readiness?.code}: ${goodExt.json?.readiness?.reason}`);
    console.log("        (honest degradation; section 5 asserts the withheld path instead)");
  }
  ...
  // Branches on publication, and BOTH branches are real assertions. When the
  // lecture is published the student must actually receive its knowledge; when
  // it is withheld the student must receive nothing at all. The one thing that
  // must never happen -- knowledge from the quarantined lecture -- is asserted
  // either way, below.
  if (published) {
    check(
      "the student CAN see knowledge from the published lecture (the path works)",
      goodUnits.length > 0,
      { status: sUnits.status, count: goodUnits.length },
    );
  } else {
    check(
      "an UNPUBLISHED lecture is withheld from the student entirely",
      sUnits.status === 403 || goodUnits.length === 0,
      { status: sUnits.status, count: goodUnits.length },
    );
  }
  check(
  ...
  check(
    published
      ? "asking a question returns a grounded answer (retrieval is alive)"
      : "asking a question returns nothing when the only lecture is unpublished",
    published
  ...
  section("6. Replay cannot be reached by accident");
  // The mechanism that produced the 2026-08-22 contamination, asserted dead
  // against the real route rather than reasoned about.
  const byName = await api(faculty.token, "POST", `/api/courses/${course.id}/lectures`, {
    title: "Control - named after a fixture",
    originalFilename: "fft-lecture-misdetected.mp3",
    fileSizeBytes: 4096, contentType: "audio/mpeg",
    checksumSha256: createHash("sha256").update("control").digest("hex"),
  });
  check(
    "a file NAMED after a fixture slug still records no replay",
    byName.status === 200 && byName.json?.replayFixture === null,
    { status: byName.status, replayFixture: byName.json?.replayFixture },
  );

  const unknown = await api(faculty.token, "POST", `/api/courses/${course.id}/lectures`, {
    title: "Control - unknown fixture",
    originalFilename: "Lecture 3 recording.mp3",
    fileSizeBytes: 4096, contentType: "audio/mpeg",
    checksumSha256: createHash("sha256").update("control-2").digest("hex"),
    replayFixture: "no-such-fixture",
  });
  check(
    "an unknown fixture slug is refused with 400, not silently ignored",
    unknown.status === 400,
