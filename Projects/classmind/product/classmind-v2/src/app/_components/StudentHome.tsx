...
import { AssignmentIcon, BookIcon, ChevronRightIcon, KeyIcon } from "@/app/_components/ui/icons";
...
// owe, and what did I miss" -- and the answer to the first half is spread
// across every course they are in. So the work comes first, whole, before any
// mention of courses: a student who has to open four courses to find out
// whether they have homework does not have a home screen, they have a
// directory.
//
// NOTHING teacher-facing may appear on this screen. No status notes ("waiting
// for your review"), no unreviewed content, no review affordances. The single
// thing a student is told about unreviewed work is a NUMBER, and the wording
// makes clear whose job it is: waiting for the lecturer, not for them.
...
// The honesty sentence, in one place because it is said in three.
//
// "Nothing to do" and "your lecturer has not looked yet" are different facts
// and a student acts differently on each. Conflating them is the failure this
...
export default function StudentHome({ eyebrow, data, onJoinCourse }: Props) {
  ...
  function subtitle(): string {
    ...
    return `${courses}. ${plural(data.recentLecturesTotal, "lecture", "lectures")} you can read back.`;
  }

  if (!courseCount) {
    return (
      <Page>
        <PageHeader eyebrow={eyebrow} title="Your work" subtitle={subtitle()} />
        <EmptyState
  ...
  return (
    <Page>
      ...
      {/* The dominant content of the screen, and the reason it exists. */}
      <Section
          ...
          <>
            ...
            {data.todoTotal > data.todo.length ? (
              <p className="mt-4 text-[13px] text-ink-faint">
                Showing {data.todo.length} of {data.todoTotal}. The rest are inside their courses.
              </p>
            ) : null}
            {awaiting ? (
      ...
      {data.recentLectures.length ? (
        ...
        <Section title="Recently added">
          <EmptyState
            ...
            description="When your lecturer publishes one, it appears here and you can read it back in full."
          />
        </Section>
      )}
...
// One obligation, whole.
//
// The steps and the "not specified" list are on the card rather than one click
// away, because they are the difference between knowing an assignment exists
// and knowing what it asks for -- and because `unspecified` is the field that
// stops a student inferring a deadline that was never given.
function TodoCard({ item }: { item: TodoItem }) {
  return (
    <Card>
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        {/* Amber means "this one is about time". If every kind were amber the
            colour would stop meaning anything, and the one item with a date on
            it would look exactly like the four without. */}
      ...
      <h3 className="mt-3.5 text-[17px] leading-snug font-semibold tracking-[-0.012em] text-balance text-ink sm:text-[19px]">
        {item.title}
      </h3>
