...
export async function GET(_r: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const visible = isOwner
      ? lectures ?? []
      : await (async () => {
          // Same reason as above: a lecture a student can see but not open is a
          // dead end. lectureVisibleToStudents is the ONE definition of this.
          const rows = await fetchLectureGateRows({ courseId: id });
          const visible = new Set(rows.filter(lectureVisibleToStudents).map((r) => r.id));
          return (lectures ?? []).filter((l) => visible.has(l.id as string));
        })();
