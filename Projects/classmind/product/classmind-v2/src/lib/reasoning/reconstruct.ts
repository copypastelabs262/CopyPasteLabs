import "server-only";
import type { NormalizedTranscript } from "../transcript/types.ts";
import { categoryOf, type ExtractionCandidate } from "../extraction/index.ts";
import { getReasoningProvider } from "./index.ts";
import type { ReasoningProvider } from "./types.ts";
import { sameSpan, spanOf } from "./span.ts";
...
export interface ReconstructionResult {
  ...
  method: string;
  version: string;
  // Did this pass read the WHOLE lecture?
  //
  // False when any window failed, and false when the transcript could not be
  // divided into windows at all. It is the difference between "there was
  // nothing to find" and "we never managed to look", and every caller that
  // stores or publishes the result needs it: an incomplete pass has seen only
  // part of the lecture, so its silence about the rest means nothing.
  //
  // This exists because it was missing. Per-window failures were collected into
  // `stats.failures` and nothing read them, so a run in which every single call
  // came back empty returned `items: []` and no error, and the caller could not
  // tell that from a lecture with no content in it.
  complete: boolean;
  ...
  stats: {
    ...
    //
    // The provider has always returned usage on every response and runWindow
    // threw it away, so a pass could be re-run indefinitely with nothing in the
    // product able to say what any of it cost. These are what the processing
    // ledger records.
    //
    // callsWithUsage is not decoration: a provider that reports no usage must
    // be distinguishable from one that used no tokens. Zero here means the
    // token sums are unknown, not that they are zero.
    providerId: string;
    model: string;
    promptTokens: number;
...
interface Window {
  startMs: number;
  endMs: number;
  // MARKER-FREE speech. See below.
  text: string;
  // Where `text` sits inside the spoken projection, so a quote can be required
  // to come from THIS window rather than from anywhere in the lecture.
  spokenStart: number;
  spokenEnd: number;
}

// The excerpt covering a time range.
//
// THE EXCERPT IS CUT FROM THE SPOKEN PROJECTION, NOT FROM THE TRANSCRIPT TEXT.
//
// The normalized transcript interleaves [mm:ss] markers for reading. Cutting
// the window out of it handed the model an excerpt with markers inside, then
// asked it to copy quotes "character-for-character" -- so any quote spanning a
// segment boundary came back containing a timestamp, and verification searches
// marker-free speech, so it could never be found. The item carrying it was
// discarded as unverifiable although the model had done exactly as it was told.
//
// That is precisely the multi-sentence case this layer exists for: an
// obligation assembled from four sentences is the one most likely to be quoted
// across a boundary. Feeding the model the same text verification searches
// removes the whole class of failure rather than trying to repair its output.
function windowFor(
  ...
  const from = spoken.segStart[firstIdx];
  const full = spoken.segStart[lastIdx] + t.segments[lastIdx].text.length;
  const to = Math.min(full, from + MAX_WINDOW_CHARS);
  return {
    startMs: t.segments[firstIdx].startMs,
    endMs: t.segments[lastIdx].endMs,
    text: spoken.text.slice(from, to),
    spokenStart: from,
    spokenEnd: to,
  };
}
...
interface Spoken {
  ...
  segAt: Int32Array;
  // For each SEGMENT, where its text begins in `text`. This is what lets a
  // window be cut from the spoken projection rather than from the transcript.
  segStart: Int32Array;
}
...
function buildSpoken(t: NormalizedTranscript): Spoken {
  ...
  t.segments.forEach((s, i) => {
    ...
    segStart.push(real.length);
    parts.push(s.text);
    for (let k = 0; k < s.text.length; k += 1) { real.push(s.charStart + k); seg.push(i); }
  });
  return {
    text: parts.join(""),
    realAt: Int32Array.from(real),
    segAt: Int32Array.from(seg),
    segStart: Int32Array.from(segStart),
  };
}
...
// `bounds` restricts the search to the window the model was actually shown.
//
// Without it the guarantee was weaker than the one this layer advertises:
// quotes were matched against the WHOLE transcript, so an item reconstructed
// from the excerpt at 04:00 could be admitted on the strength of a sentence
// spoken at 41:00 that the model never saw. That combination -- bounded input,
// unbounded verification -- is how a coincidence becomes evidence. Verification
// now asks the question the architecture claims it asks: does this quote occur
// in THIS excerpt?
function locateQuote(
  ...
  // A lecturer repeats sentences, so the first occurrence is not necessarily
  // the one inside the window. Walk them until one lands in range.
  for (let at = collapsed.norm.indexOf(needle); at !== -1; at = collapsed.norm.indexOf(needle, at + 1)) {
    const from = collapsed.map[at];
    const to = collapsed.map[Math.min(at + needle.length - 1, collapsed.map.length - 1)] + 1;
    if (bounds && (from < bounds.from || to > bounds.to)) continue;
...
//
// The identity rule itself lives in ./span.ts, because the same question is
// asked again later against knowledge already stored for this lecture -- see
// planKnowledgeWrite. Two definitions of "these are the same obligation" would
// eventually disagree, and the run that merged them would not be the run that
// stored them.
function dedupeByEvidence(items: ReconstructedItem[]): ReconstructedItem[] {
  ...
  const kept: { item: ReconstructedItem; span: ReturnType<typeof spanOf> }[] = [];
  for (const item of ranked) {
    const span = spanOf(item.evidence);
    if (kept.some((k) => sameSpan(k.span, span))) continue;
    kept.push({ item, span });
  }
...
// `provider` is injectable for one reason: without it, none of the properties
// this layer is built on -- quotes verified against their own window, four
// sentences becoming one item, overlapping windows merging rather than
// double-reporting -- can be checked without spending money on a live model and
// depending on one particular recording. A pipeline whose safety mechanism can
// only be tested in production is not a safety mechanism. Production passes
// nothing and gets the configured provider.
export async function reconstructLecture(
  ...
  async function runWindow(
    ...
    try {
      ...
      // unreadable was still generated and still billed, and a meter that only
      // counts the calls that happened to succeed understates the bill by
      // exactly the failures you most want to know about.
  ...
  // A TRANSCRIPT WITH NO TIMELINE IS A FAILURE, NOT AN EMPTY LECTURE.
  //
  // windowStarts() over a zero-length timeline returns no windows, so the pass
  // would make no calls, report no failures and return no items -- exactly the
  // signature of a lecture that genuinely contains nothing. It has happened:
  // a transcript that normalizes to a single segment ending at 0ms produces
  // precisely this. Naming it here is what stops a broken transcript being
  // published as an empty one.
  if (end <= 0) {
    stats.failures.push(
      "The transcript carries no timeline -- every segment ends at 0ms -- so it could not " +
      "be divided into windows and nothing was read from it.",
    );
  }
  ...
  return {
    ...
    // Every window returned. Anything less and this pass has not seen the whole
    // lecture, so its result may not replace one that has.
    complete: stats.failures.length === 0,
    stats,
  };
}

// Exposed for scripts/test-reconstruction.mts. These are the mechanics the
